...
const CampaignsPage = () => {
  ...
  useEffect(() => {
    const custom = JSON.parse(localStorage.getItem('custom_campaigns') || '[]');
    if (custom.length > 0) {
      setCampaigns([...custom, ...initialCampaigns]);
    }
  }, []);
        ...
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm">
          ...
          <div className="hidden md:block overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[var(--bg)]/50 border-b border-[var(--border)] text-[var(--muted)]">
                  <th className="w-14 pl-6 py-4">
                    <button 
                      onClick={toggleSelectAll}
                      className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${selectedIds.length === filteredCampaigns.length && filteredCampaigns.length > 0 ? 'bg-blue-600 border-blue-600 text-white' : 'border-[var(--border)] bg-[var(--bg)] text-transparent'}`}
                    >
                      <CheckCircle2 size={12} strokeWidth={3} />
                    </button>
                  </th>
                  <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest">Status</th>
                  <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest">Campaign Name</th>
                  <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest">Metrics</th>
                  <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-right pr-6">Performance & Actions</th>
                </tr>
              </thead>
          ...
          {/* Mobile Card View */}
          <div className="md:hidden divide-y divide-[var(--border)] bg-[var(--bg)]/30">
            {filteredCampaigns.map((camp, i) => (
              <motion.div
                key={camp.id}
                ...
                className="p-4 bg-[var(--card)] space-y-4"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <button 
                      onClick={() => toggleSelect(camp.id)}
                      className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${selectedIds.includes(camp.id) ? 'bg-blue-600 border-blue-600 text-white' : 'border-[var(--border)] bg-[var(--bg)] text-transparent'}`}
                    >
                      <CheckSquare size={12} strokeWidth={2.5} />
                    </button>
                    <div className="space-y-0.5">
                      <p className="text-xs font-bold text-[var(--text)] uppercase tracking-tight line-clamp-1">{camp.name}</p>
                      <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-widest">Started {camp.createdAt}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                     <button className={`w-8 h-4 rounded-full relative transition-all duration-500 shrink-0 ${camp.status === 'Active' ? 'bg-blue-600' : 'bg-[var(--border)]'}`}>
                        <div className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow-sm transition-all duration-500 ${camp.status === 'Active' ? 'left-[18px]' : 'left-0.5'}`} />
                     </button>
                     <div className="relative" onClick={() => setOpenActionId(openActionId === camp.id ? null : camp.id)}>
                        <button className="h-8 w-8 text-[var(--muted)]">
                          <MoreVertical size={16} />
                        </button>
                        <ActionDropdown isOpen={openActionId === camp.id} onClose={() => setOpenActionId(null)} campaign={camp} onDuplicate={handleDuplicate} onDelete={handleDelete} />
                     </div>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-2 bg-[var(--bg)] p-3 rounded-xl border border-[var(--border)]">
                   <div className="text-center space-y-0.5">
                      <p className="text-[8px] font-bold text-[var(--muted)] uppercase tracking-widest">Sent</p>
                      <p className="text-[11px] font-black text-[var(--text)]">{camp.messages}</p>
                   </div>
                   <div className="text-center space-y-0.5 border-x border-[var(--border)]">
                      <p className="text-[8px] font-bold text-[var(--muted)] uppercase tracking-widest">Replied</p>
                      <p className="text-[11px] font-black text-emerald-500">{camp.replied}</p>
                   </div>
                   <div className="text-center space-y-0.5">
                      <p className="text-[8px] font-bold text-[var(--muted)] uppercase tracking-widest">Conv</p>
                      <p className="text-[11px] font-black text-blue-600">{((camp.accepted / (camp.connections || 1)) * 100).toFixed(0)}%</p>
                   </div>
                </div>

                <div className="flex gap-2">
                   <Link href={`/dashboard/campaigns/edit/${camp.id}`} className="flex-1 h-9 bg-zinc-950 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-2">
