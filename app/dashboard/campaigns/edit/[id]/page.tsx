...
const EditCampaignPage = () => {
  ...
  useEffect(() => {
    ...
    const campaign = existing.find((c: { id: string, name?: string, searchUrl?: string, leads?: number, options?: Record<string, boolean>, steps?: Record<string, CampaignStep>, mainSequence?: string[] }) => c.id === id);
    if (campaign) {
      setCampaignName(campaign.name || "");
      setSearchUrl(campaign.searchUrl || "");
      setMaxLeads(campaign.leads || 500);
      if (campaign.options) setOptions(campaign.options);
      if (campaign.steps) setSteps(campaign.steps);
      if (campaign.mainSequence) setMainSequence(campaign.mainSequence);
    }
  }, [id]);
  ...
  const addStep = (type: StepType) => {
    ...
    const newStep: CampaignStep = { 
       ...
       type, 
       delay: 1, 
       content: type.includes("Message") || type.includes("request") ? "Hi {{name}}, ..." : "" 
    };
    setSteps(prev => ({ ...prev, [sid]: newStep }));
    setMainSequence(prev => [...prev, sid]);
  };

  const removeStep = (sid: string) => {
    setMainSequence(prev => prev.filter(mid => mid !== sid));
    setSteps(prev => {
      const next = { ...prev };
      delete next[sid];
      return next;
  ...
  const updateStep = (sid: string, updates: Partial<CampaignStep>) => {
    setSteps(prev => ({ ...prev, [sid]: { ...prev[sid], ...updates } }));
  };
  ...
  const handleSave = () => {
    ...
    const existing = JSON.parse(localStorage.getItem('custom_campaigns') || '[]');
    const updated = existing.map((c: { id: string }) => {
      if (c.id === id) {
        return {
          ...c,
          name: campaignName,
      ...
      <header className="h-20 bg-[var(--card)] border-b border-[var(--border)] px-6 md:px-10 flex items-center justify-between sticky top-0 z-[100] transition-colors">
         <div className="flex items-center gap-5 overflow-hidden">
            ...
            <div className="flex flex-col text-left">
               <span className="text-[9px] font-black uppercase tracking-[0.3em] text-blue-600 mb-0.5 leading-none">Campaign Editor</span>
               <h1 className="text-base font-black text-[var(--text)] uppercase tracking-tight truncate max-w-[150px] md:max-w-xs">{campaignName || "Draft Automation"}</h1>
            </div>
         </div>
         ...
         <div className="flex items-center gap-3">
            <button 
               onClick={() => {
                 ...
                 else if (wizardStep === 'finalize' && campaignName) handleSave();
               }}
            ...
            >
               <span>{wizardStep === 'finalize' ? 'Apply & Save' : 'Continue'}</span> 
              ...
              >
                 <div className="w-full max-w-4xl space-y-10">
                   <div className="text-center space-y-2 mb-8">
                      ...
                      <h3 className="text-3xl font-black uppercase tracking-tight text-[var(--text)]">Identify your <span className="opacity-20 italic">Ideal Leads.</span></h3>
                   </div>
                         ...
                         <div className="bg-[var(--card)] border border-[var(--border)] rounded-3xl p-8 space-y-6 shadow-sm">
                            <div className="flex items-center gap-3">
                               ...
                               <div>
                                  ...
                                  <p className="text-[10px] font-medium text-[var(--muted)] opacity-50">Paste a people search or Sales Navigator URL</p>
                               </div>
                            </div>
                            <textarea 
                               value={searchUrl}
                      ...
                      <div className="lg:col-span-4 space-y-6 text-left">
                         <div className="bg-[var(--card)] border border-[var(--border)] rounded-3xl p-8 space-y-8 shadow-sm">
                            ...
                            <div className="relative pt-2">
                               ...
                               />
                               <div className="flex justify-between mt-3 text-[8px] font-black uppercase tracking-widest text-[var(--muted)] opacity-30">
                                  <span>10 Leads</span>
              ...
              >
                 <div className="w-full max-w-2xl space-y-12">
                   <div className="text-center space-y-2 mb-8">
                      ...
                      <h3 className="text-3xl font-black uppercase tracking-tight text-[var(--text)]">Strategic <span className="opacity-20 italic">Timeline.</span></h3>
                   </div>
                    ...
                    <div className="bg-[var(--card)] border border-[var(--border)] p-12 space-y-10 rounded-[3rem] shadow-sm relative text-left">
                       <div className="space-y-10">
                          ...
                          <div className="pt-6">
                            ...
                            >
                                <span className="text-[13px] font-black uppercase tracking-[0.3em]">Update & Save Strategy</span>
