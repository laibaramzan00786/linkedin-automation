...
const EditCampaignPage = () => {
  ...
  useEffect(() => {
    ...
    const campaign = existing.find((c: { id: string }) => c.id === id);
    if (campaign) {
    ...
    <div className="flex flex-col h-screen bg-[var(--bg)] overflow-hidden font-sans">
      ...
      <header className="h-20 bg-[var(--card)]/80 backdrop-blur-xl border-b border-[var(--border)] px-4 md:px-10 flex items-center justify-between sticky top-0 z-[100]">
         ...
         <div className="flex items-center gap-3">
            <button onClick={handleSave} className="h-12 px-8 bg-linear-to-r from-blue-600 to-indigo-600 text-white text-[11px] font-black uppercase tracking-[0.15em] rounded-2xl hover:translate-y-[-2px] hover:shadow-2xl hover:shadow-blue-500/30 transition-all flex items-center gap-3 shadow-xl active:scale-95 group">
               <span>Save Campaign</span> 
               <ChevronRight size={18} className="group-hover:translate-x-1 transition-transform" />
            </button>
            <button onClick={() => router.push('/dashboard/campaigns')} className="w-12 h-12 rounded-2xl border border-[var(--border)] flex items-center justify-center text-[var(--muted)] hover:bg-rose-500 hover:text-white hover:border-rose-500 transition-all active:scale-90">
               <X size={24} />
            </button>
         </div>
      </header>
                  ...
                  {builderMode === 'landing' ? (
                    ...
                    <div className="flex-1 flex flex-col relative min-h-0">
                        ...
                        <div className="flex-1 overflow-auto relative custom-scrollbar flex flex-col min-h-0 bg-zinc-50/50 dark:bg-black/10">
                           ...
                           >
                              <div className="px-10 py-5 bg-[var(--card)] border-2 border-[var(--border)] rounded-3xl text-[13px] font-black uppercase tracking-[0.25em] text-[var(--text)] shadow-2xl animate-in fade-in zoom-in-95 duration-1000 flex items-center gap-5 group/start hover:scale-105 hover:border-blue-600/30 transition-all cursor-default">
                                 <div className="relative">
                                    <div className="w-3 h-3 rounded-full bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.8)] animate-pulse" />
                                    <div className="absolute inset-x-0 inset-y-0 w-3 h-3 rounded-full bg-emerald-500 animate-ping opacity-20" />
                                 </div>
